...
### Date and Author
# author: simon.crameri@usys.ethz.ch
# date: Mar 2022

### Install needed packages
need.pkg <- c("readxl","adegenet","ape","cluster","raster","terra","ConR","alphahull","tmap","concaveman","rmapshaper")
for (pkg in need.pkg) {if (! pkg %in% installed.packages()) install.packages(pkg)}
...
### Parameters
# public paths
p.morph <- p.eco <- p.specimens <- "SupplementaryMaterial.xlsx"

# # hidden paths
# p.data.eco <- "data/specimens_433.txt" # includes precise geo-coordinates [not public]
# load("data/data.rda") # data: non-public specimen data (with coordinates) downloaded from Tropicos.org
# p.rasters <- "data/"
# p.morph.all <- "data/charactertable.xlsm" # includes repeated measurements on all organs and individuals [median values reported]
# 
# # spatial rasters (see Material and Methods for sources)
# r.longlat <- rast(file.path(p.rasters, "Ecology/MadagascarForestModel/RasterData/terrain/MDG_LongLat_utm_30.tif")) # two layers
# r.prec.annual <- rast(file.path(p.rasters, "Ecology/DataRastersVectors/Climate/CHELSA_Madagascar/terra/CHELSA_Bio12_Annual_Precipitation_utm_30.tif"))
# r.prec.seasonality <- rast(file.path(p.rasters, "Ecology/DataRastersVectors/Climate/CHELSA_Madagascar/terra/CHELSA_Bio15_Precipitation_Seasonality_utm_30.tif"))
# r.temp.annual <- rast(file.path(p.rasters, "Ecology/DataRastersVectors/Climate/CHELSA_Madagascar/terra/CHELSA_Bio01_Annual_Mean_Temperature_utm_30.tif"))
# r.temp.seasonality <- rast(file.path(p.rasters, "Ecology/DataRastersVectors/Climate/CHELSA_Madagascar/terra/CHELSA_Bio04_Temperature_Seasonality_utm_30.tif"))
# r.isothermality <- rast(file.path(p.rasters, "Ecology/DataRastersVectors/Climate/CHELSA_Madagascar/terra/CHELSA_Bio03_Isothermality_utm_30.tif"))
# r.elevation <- rast(file.path(p.rasters, "Ecology/MadagascarForestModel/RasterData/terrain/MDG_alt_utm_30.tif"))
# r.aspect <- rast(file.path(p.rasters, "Ecology/MadagascarForestModel/RasterData/terrain/MDG_aspect_utm_30.tif"))
# r.slope <- rast(file.path(p.rasters, "Ecology/MadagascarForestModel/RasterData/terrain/MDG_slope_utm_30.tif"))
# r.dist2Coast <- rast(file.path(p.rasters, "Ecology/MadagascarForestModel/RasterData/terrain/MDG_dist2Coast_utm_30.tif"))
# r.dist2InlandWaters <- rast(file.path(p.rasters, "Ecology/MadagascarForestModel/RasterData/terrain/MDG_dist2InlandWaters_utm_30.tif"))
# r.geol <- rast(file.path(p.rasters, "Ecology/MadagascarForestModel/RasterData/terrain/MDG_geol_utm_30.tif"))
# r.vege <- rast(file.path(p.rasters, "Ecology/DataRastersVectors/Vegetation/AtlasOfTheVegetationOfMadagascar/AtlasOfTheVegetationOfMadagascar2007_utm_30.tif"))
# 
# # bind rasters
# r.eco <- c(r.longlat, r.prec.annual, r.prec.seasonality, r.temp.annual, r.temp.seasonality, r.isothermality, r.elevation,
#   r.aspect, r.slope, r.dist2Coast, r.dist2InlandWaters, r.geol, r.vege)
# 
# terra::writeRaster(r.eco, filename = "r.eco_utm_30.tif",
#                    filetype = "GTiff", overwrite = TRUE,
#                    gdal = c("INTERLEAVE=BAND","COMPRESS=LZW"))
# 
# # spatial rasters and vectors (used only for Figure 3D)
# for2017 <- raster(file.path(p.rasters, "Ecology/MadagascarForestModel/RasterData/forest/for2017.tif"))
# r.for2017 <- rast(file.path(p.rasters, "Ecology/MadagascarForestModel/RasterData/forest/for2017.tif"))
# r.alt <- rast(file.path(p.rasters, "Ecology/DataRastersVectors/DIVA-GIS/MDG_alt/MDG_alt.tif"))
# r.for2017.lowres <- project(x = r.for2017, y = r.alt, method = "near")
# terra::writeRaster(r.for2017.lowres, filename = file.path(p.rasters, "Ecology/MadagascarForestModel/RasterData/forest/for2017_longlat_30arcsec.tif"),
#                    filetype = "GTiff", overwrite = TRUE,
#                    gdal = c("INTERLEAVE=BAND","COMPRESS=LZW"))
# r.for2017.lowres <- raster(file.path(p.rasters, "Ecology/MadagascarForestModel/RasterData/forest/for2017_longlat_30arcsec.tif"))
# eco2017 <- readRDS(file.path(p.rasters, "Ecology/DataRastersVectors/Ecoregions/Madagascar_ecoregions2017.rds"))
# alt <- raster(file.path(p.rasters, "Ecology/DataRastersVectors/DIVA-GIS/MDG_alt/MDG_alt.grd"))
# border <- readRDS(file.path(p.rasters, "Ecology/DataRastersVectors/AdministrativeBoundaries/Madagascar_border.rds"))
# regions <- readRDS(file.path(p.rasters, "Ecology/DataRastersVectors/AdministrativeBoundaries/MDG_regions.rds")) # 23 regions
# pas <- readRDS(file.path(p.rasters, "Ecology/DataRastersVectors/ProtectedAreas/MDG_PA_202102_124.rds"))

# categorical variables
categorical <- c("geol","vege")
min.frac.dummy <- 0.05 # minimum level frequency: at least this fraction of observations need to be at this categorical level for that level to be kept
na.strings <- c("NA","",NA)
...
# classes
spec.morpho <- c("aff._chapelieri_coriaceous","aff._chapelieri_coriaceous_large","aff._chapelieri_Mahabo","aff._chapelieri_Manombo","aff._chapelieri_N","aff._chapelieri_NE","aff._chapelieri_S","aff._chapelieri_Vohemar","chapelieri","cf._chapelieri","pterocarpiflora", # chapelieri sensu lato
                 "louvelii_louvelii","louvelii_mananara","louvelii_glabra","louvelii","aff._louvelii_Mananara","aff._louvelii_Mananjary", # louvelii sensu lato (excluding aff._louvelii_PAL because that is a hybrid btw hassoldiae and aff._normandii_PAL)
...
spec.map <- c(spec.morpho,
              "bernardii") # undescribed SAVA material
  ...
  switch(fun, 
         ...
         class.mean = {
           ...
           for (i in levels(classes)) {
             dfsub <- df[classes == i,]
             tmp <- apply(dfsub, MARGIN = 2, FUN = impute, fun = "mean", simplify = T)
...
# match specimens and taxa from database 
get.meta <- function(df, data) {
  coll <- sub("Service Forestier Madagascar 9144", "Service Forestier Madagascar, Capuron 9144", sub(" SF$| RN$", "", rownames(df)))
  stopifnot(all(coll %in% data$Collection))
  df.meta <- data[match(coll, data$Collection), c("SpecimenID","DGroup","Genus","Species","CurrentDetermination","DeterminationBy","LatitudeDecimal","LongitudeDecimal","MinimumElevation","MinimumYear")]
  df.meta
}
...
# dd.all <- data.frame(readxl::read_excel(p.morph.all, na = c("", "NA")))
# 
# # subset rows
...
# # select species
# dd.all <- dd
# dd <- subset(dd.all, Species %in% spec.morpho)
# 
# # combine sensu lato
# sp.morpho <- spec.morpho
# sp.map <- spec.map
# for (i in s.lat) {
#   dd$Species[grep(i, dd$Species)] <- paste(i, "s.l.")
#   sp.morpho[grep(i, sp.morpho)] <- paste(i, "s..")
#   sp.map[grep(i, sp.map)] <- paste(i, "s.l.")
# }
# sp.morpho <- sort(unique(paste0("D. ", gsub("_", " subsp. ", sp.morpho))))
# sp.map[grep("racemosa", sp.map)] <- "sp. (SAVA material)"
# sp.map <- sp.map[!sp.map %in% "pterocarpiflora"]
...
# 
# df.meta <- get.meta(df = dd.leaf, data = data)
# table(TAXON = dd.leaf.spec, Species = df.meta$Species) # all correct
# 
# dd.leaf.cols <- col.pal[sp.map %in% sp.morpho]
# 
# dd.leaf$TAXON <- dd.leaf.spec
# rownames(dd.leaf) <- gsub(" RN$", "", gsub(" SF$", "", rownames(dd.leaf)))
# write.table(dd.leaf, file = p.morph.leaf, row.names = TRUE, quote = FALSE, sep = "\t")

dd.leaf <- data.frame(readxl::read_excel(p.morph, sheet = "Supplementary Material 3", skip = 2, na = "NA"))
rownames(dd.leaf) <- dd.leaf[,1] ; dd.leaf <- dd.leaf[,-1]
colnames(dd.leaf) <- gsub("[..].*", "", colnames(dd.leaf))

dd.leaf.spec <- factor(dd.leaf$TAXON) ; dd.leaf <- dd.leaf[,!names(dd.leaf) %in% "TAXON"]
...
res.leaf <- ordplot(df = dd.leaf, x = 1, y = 2, classes = dd.leaf.spec, cols = dd.leaf.cols,
                    ordratio = ordratio, asymm = asymm, labels = F, pcoa.flipx = -1, pcoa.flipy = -1,
...
# 
# df.meta <- get.meta(df = dd.flower, data = data)
# table(TAXON = dd.flower.spec, Species = df.meta$Species) # all correct
# 
...
dd.flower <- data.frame(readxl::read_excel(p.morph, sheet = "Supplementary Material 4", skip = 2, na = "NA"))
rownames(dd.flower) <- dd.flower[,1] ; dd.flower <- dd.flower[,-1]
colnames(dd.flower) <- gsub("[..].*", "", colnames(dd.flower))

dd.flower.spec <- factor(dd.flower$TAXON) ; dd.flower <- dd.flower[,!names(dd.flower) %in% "TAXON"]
...
### Load spatial raster data
# r.eco <- rast("r.eco_utm_30.tif")
# names(r.eco) <- c(geovars, ecovars)
# 
# ### Read tropicos data
...
# dsub$Species[dsub$Species %in% "bernardii"] <- "sp. (SAVA material)"
# 
# dsub <- dsub[!duplicated(dsub[,c(classvar,geovars)]),] # remove conspecific specimens georeferenced to the same coordinates
# dim(dsub) # 433 with precise coordinates
# write.table(dsub, file = paste0("data/specimens_", nrow(dsub), ".txt"), sep = "\t", row.names = FALSE, quote = FALSE)
# 
# 
# ### Read tropicos data
# dsubGEO <- read.delim(p.data.eco) # with precise coordinates
# 
# ### Subset tropicos data
# # select specimens with accurate georeferencing
# dsub <- subset(dsubGEO, !is.na(LatitudeDecimal) & Coordinate.Method %in% accurate.geo)
# 
...
# dim(dsub) # 344 (includes new collections)
# 
# collection330 <- data.frame(readxl::read_excel(p.eco, sheet = "Supplementary Material 5", skip = 2, na = "NA"))$Collection
# dsub <- dsub[collection330,]
# dim(dsub) # 330 (425 dataset)
# 
# ### Extract ecological variables
# # spatial vector of sample coordinates
# # v.eco <- vect(x = dsub[,geovars], geom = geovars, type = "points", crs = "+init=epsg:4326") # version early 2021
# v.eco <- vect(x = dsub[,geovars], geom = geovars, crs = "+init=epsg:4326")
# v.eco <- terra::project(v.eco, r.eco) # same projection as rasters (UTM)
# 
# # extract
# deco <- dsub
# for (ecovar in ecovars) {
#   deco[,ecovar] <- terra::extract(x = r.eco[[ecovar]], y = v.eco, method = ifelse(ecovar %in% categorical, "simple", "bilinear"))[,2]
# }
# 
# 
...
# dd.SM1 <- merge(dsubGEO, deco, by = intersect(names(dsubGEO), names(deco)), all.x = TRUE, all.y = TRUE, sort = FALSE)
...
# write.table(dd.SM1[,c("SpecimenID","Collection","Genus","Species","CurrentDetermination","DeterminationBy","Minimum.Year","Coordinate.Method")], file = "SupplementaryMaterial1.csv", sep = "\t", quote = FALSE, row.names = FALSE)
# 
# 
...
#   deco <- factor2dummy(df = deco, factorname = i, keep.levels = dummy[dummy %in% names(tab.dummy)])
# }
# 
# 
# ### Fix column names
# colnames(deco) <- gsub("geol_4", "LITH_Metasediment", gsub("geol_6", "LITH_Silicic", gsub("geol_7", "LITH_Metaigneous", gsub("geol_9", "LITH_Volcanic", gsub("geol_18", "LITH_Alluvium",
#                    gsub("vege_14", "VEGE_Humid", gsub("vege_15", "VEGE_Littoral", gsub("vege_16", "VEGE_HumidDegr",
...
# 
# ss.eco <- subset.data(df = deco, grep.keep = var.eco, grep.rm = var.eco.excl, tableplot = FALSE)
...
# dd.eco <- dd.eco[,c("TAXON",names(dd.eco)[!names(dd.eco) %in% "TAXON"])]
# 
# dd.eco.meta <- get.meta(df = dd.eco, data = data)
# table(TAXON = dd.eco$TAXON, Species = dd.eco.meta$Species) # new
# 
# write.table(dd.eco, file = "SupplementaryMaterial5.txt", sep = "\t", quote = FALSE, row.names = TRUE) # scaled coordinates

dd.eco <- data.frame(readxl::read_excel(p.eco, sheet = "Supplementary Material 5", skip = 2, na = "NA"))
rownames(dd.eco) <- dd.eco[,1] ; dd.eco <- dd.eco[,-1]
colnames(dd.eco) <- gsub("[..].*", "", colnames(dd.eco))
# dd.eco <- read.delim(p.eco, row.names = 1) # 330, scaled coordinates
...
res.eco <- ordplot(df = dd.eco, x = 1, y = 2, classes = dd.eco.spec, cols = dd.eco.cols,
                   pca = TRUE, pcoa = FALSE, pca.flipx = -1, pca.flipy = -1,
                   hulls = TRUE, concavity = 1E06, voronoi = FALSE,
...
# 
# # read data
# dsub.map <- dsub.map425 <- read.delim(p.data.eco) # 425 with precise coordinates (not public)
# 
# dsub.map[dsub.map$Collection == "Geay 7859","Species"] <- "louvelii s.lat."
# 
# # filter out doubtful specimens
# dsub.map <- subset(dsub.map, !Collection %in% c("Service Forestier Madagascar 38-R-118","Service Forestier Madagascar 34291"))
# # dsub.map <- subset(dsub.map, Coordinate.Method %in% c("GPS","google earth","map","post facto, high","corrected post facto"))
# dim(dsub.map) # 423
# 
# # assign islands
# dsub.map[grepl("chapelieri", dsub.map$Species) & dsub.map$LatitudeDecimal > -14.5 &  dsub.map$LongitudeDecimal > 49,"Species"] <- "chapelieri s.lat. II"
# dsub.map[grepl("chapelieri", dsub.map$Species) & dsub.map$LatitudeDecimal > -14.5 &  dsub.map$LongitudeDecimal <= 49,"Species"] <- "chapelieri s.lat. II"
# dsub.map[grepl("louvelii", dsub.map$Species) & dsub.map$LatitudeDecimal > -17.5,"Species"] <- "louvelii s.lat. II"
# dsub.map[grepl("louvelii", dsub.map$Species) & dsub.map$LatitudeDecimal < -20, "Species"] <- "louvelii s.lat. III"
# # calculate EOO
...
# # sp.order <- c(1,2,3,5,4,7,6,8)
# # sp.order <- c(1,2,3,4,5,6,8,7,10,9,11)
# sp.order <- c(1,2,3,4,5,7,6,9,8,10)
...
#   
#   switch(i,
#          "chapelieri s.lat." = {alpha = 10; buff.alpha = 300/3600}, # alpha = 5
#          "chapelieri s.lat. II" = {alpha = 0.5;buff.alpha = 300/3600},
#          "chapelieri s.lat. III" = {alpha = 0.5;buff.alpha = 300/3600},
#          "louvelii s.lat." = {alpha = 10; buff.alpha = 300/3600}, # alpha = 1
#          "louvelii s.lat. II" = {alpha = 10; buff.alpha = 300/3600}, # alpha = 1
#          "louvelii s.lat. III" = {alpha = 20; buff.alpha = 300/3600}, # alpha = 1
...
#                              "Manombo","Tolagnaro","Mandena","Sainte Luce","Tsitongambarika","Sahafina","Ambila-Lemaitso","Manakara","Analalava","Vangaindrano","Mananjary"),
...
# shape.omit <- sp.place$Place[sp.place$Type %in% c("Protected Area") | sp.place$Place %in% "Antongil\nBay"]
# 
# col.pal <- c("#A6CEE3","#A6CEE3","#A99099","#A99099","#A99099","#4F9F3B","#E93E3F","#FDAC4F","#B15928","#FFFF99")
# 
# # plot
...
#   tm_shape(r.for2017.lowres, raster.downsample = T) +
...
#   tm_shape(shp = pas[!pas$GROUP == "MARINE",], simplify = 1) +
